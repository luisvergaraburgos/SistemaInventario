...
namespace SistemaInventario.Areas.Inventario.Controllers
{
    ...
    public class CarroController : Controller
    {
        ...
        public async Task<IActionResult> menos (int carroId)
        {
            ...
            {
                // Removemos el registro del carro de compras y actualizamos la seción
                var carroCompraLista = await _unidadTrabajo.CarroCompra.ObtenerTodos(c => c.UsuarioAplicacionId == carroCompras.UsuarioAplicacionId);
                var numeroProductos = carroCompraLista.Count();
        ...
        {
            // Remueve el registro del carro de compras y actualiza la sesión
            var carroCompras = await _unidadTrabajo.CarroCompra.ObtenerPrimero(c=>c.Id == carroId);
            var carroCompraLista = await _unidadTrabajo.CarroCompra.ObtenerTodos(c => c.UsuarioAplicacionId == carroCompras.UsuarioAplicacionId);
            var numeroProductos = carroCompraLista.Count();
